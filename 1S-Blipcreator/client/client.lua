...
-- Admin check before command execution
RegisterCommand('blipscreator', function()
    -- Request server-side admin check
    TriggerServerEvent('1S-Blips:adminCheck')
end, false)

-- Handle admin check result from the server
RegisterNetEvent('1S-Blips:adminCheckResult', function(isAdmin)
    ...
    -- Proceed with the blip creation dialog if admin
...
-- Load blips when player spawns or resource starts
